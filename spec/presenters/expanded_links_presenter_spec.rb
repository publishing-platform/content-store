...
RSpec.describe ExpandedLinksPresenter do
  describe ".present" do
    ...
    let(:prefix) { PublishingPlatformLocation.website_root }
    ...
    context "production environment" do
      let(:production_prefix) { "https://www.publishing-platform.co.uk" }
      let(:expected) do
        {
          link_group: [
            a_hash_including(
      ...
      before { allow(PublishingPlatformLocation).to receive(:website_root).and_return(production_prefix) }

      it { is_expected.to include expected }
    end

    context "development environment" do
      let(:development_prefix) { "http://dev.publishing-platform.co.uk" }
      let(:expected) do
        {
          link_group: [
            a_hash_including(
      ...
      before { allow(PublishingPlatformLocation).to receive(:website_root).and_return(development_prefix) }
