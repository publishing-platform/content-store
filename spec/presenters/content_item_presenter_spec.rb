...
describe ContentItemPresenter do
  ...
  let(:item) { build(:content_item, schema_name: "answer", expanded_links:) }
  ...
  it "validates against the schema" do
    content_item = create(:content_item, :with_content_id, schema_name: "answer", document_type: "answer")

    presented = ContentItemPresenter.new(content_item, api_url_method).as_json

    expect(presented.to_json).to be_valid_against_frontend_schema("answer")
  end
  ...
  it "renders timestamps in iso8601 format" do
    content_item = create(:content_item, first_published_at: Time.zone.now)
    presented = ContentItemPresenter.new(content_item, api_url_method).as_json
    %w[updated_at public_updated_at first_published_at].each do |key|
  ...
  context "when some timestamps are nil" do
    it "renders them as nil" do
      content_item = build(:content_item, public_updated_at: nil, first_published_at: nil)
      presented = ContentItemPresenter.new(content_item, api_url_method).as_json
      %w[public_updated_at first_published_at].each do |key|
