...
describe "content item write API", type: :request do
  ...
  describe "creating a new content item" do
    context "when the user is not authenticated" do
      around do |example|
        ClimateControl.modify(PUBLISHING_PLATFORM_SSO_MOCK_INVALID: "1") { example.run }
      end
  ...
  describe "updating an existing content item" do
    ...
    context "when the router-api is unavailable" do
      let!(:stub) do
        stub_http_request(:any, /^#{Regexp.escape(PublishingPlatformApi::TestHelpers::Router::ROUTER_API_ENDPOINT)}\//)
      ...
      it "fails to update content item" do
        ...
        expect { put_json "/content/vat-rates", @data }
          .to raise_error(PublishingPlatformApi::HTTPInternalServerError)
        @item.reload
